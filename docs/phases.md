...
- [ ] Google OAuth（認可コード + PKCE）、セッション Cookie 発行、`/auth/me`
- [ ] `/sync/push` の適用ロジック（LWW・seq 採番・冪等性・所有権チェック）
- [ ] `/sync/pull`
- [ ] 循環参照チェックと親子完了制約のバリデーション
- [ ] React 側の骨組み：2 ペインレイアウト、ルーティング、ダーク / ライト切替
- [ ] フォルダ / リストのツリー表示と CRUD
- [ ] タスク一覧・作成・編集・完了・削除
- [ ] 優先度、期限（終日 / 時刻あり）、タグ
- [ ] 無制限ネストのサブタスク表示（`WITH RECURSIVE`）
- [ ] ソート切替（カスタム / 期限 / 優先度 / 名前）
  - 名前順は `Intl.Collator(numeric: true)`
  - 期限順は `ORDER BY due_at IS NULL, due_at`（NULL を末尾へ）
- [ ] スマートリスト（今日 / 明日 / 今週 / 期限なし / 全て / 完了済み）
- [ ] 期限切れの「今日」への繰り越し表示（`due_at` は書き換えない）
- [ ] キーボードショートカット（既定キーマップ、`?` で一覧、入力欄フォーカス時は無効化）
- [ ] キーマップのカスタマイズ UI（`user_settings.keymap_json`、競合警告）
- [ ] SameSite=Strict Cookie、`email_verified` 検証、主要エンドポイントのレート制限
...
- [ ] rrule.js 導入。RRULE の生成 UI（毎日 / 毎週 / 毎月 / カスタム）
- [ ] 繰り返しタスクの完了処理
  - 元の予定日基準で次の occurrence を算出（`dtstart` をズラさない）
  - 過去分は生成せず、常に直近 1 件のみ表示
- [ ] FTS5（trigram）の索引と同期トリガーの動作確認
- [ ] `/search` 実装。**3 文字未満は LIKE にフォールバック**
- [ ] 検索 UI（タスクとメモを横断）
...
- [ ] メモ機能（一覧・作成・編集・色・ピン留め）。タスクとは完全に別画面
- [ ] 添付：クライアント側の縮小・WebP 変換・SHA-256 計算
- [ ] `POST/GET /attachments/{sha256}`、`X-Accel-Redirect` 配信
- [ ] オフライン時の Blob 保持と復帰時アップロード（**実体 → メタデータの順**）
- [ ] ポモドーロタイマー（おまけ機能。実績記録なし）
- [ ] Web Push：VAPID 鍵の読み込み、購読登録、送信ワーカー
- [ ] 期限リマインダーの予約と、期限変更時の予約入れ直し
- [ ] 404 / 410 が返った購読の自動削除
