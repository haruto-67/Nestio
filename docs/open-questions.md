...
カンバン/カレンダー/ガントチャートのどれを指しているか、あるいは他の形式かが不明だったため、
改修3回目では実装を見送り`manual`タグ付きの質問を残した。
改修4回目でユーザーから「カンバン」「カレンダー」の両方が明示的に要望されたため実装した。
**カンバン**：ワークフロー用のステータス列は`docs/schema.sql`に存在せず、追加は「勝手に変更しない」
方針に抵触するため、既存の`priority`（優先度）を列の軸として使う設計にした。カードのドラッグ&ドロップで
優先度を変更できる（`apps/web/src/features/tasks/KanbanBoard.tsx`）。
**カレンダー**：月グリッド表示。日付セルへのドラッグ&ドロップでその日の終日タスクとして期限を設定する
（`due_at`/`due_date`は排他のため`due_at`はクリアする）。既存のICSフィード機能は「外部カレンダーアプリへの
購読提供」が役割で、アプリ内での月表示ナビゲーションとは役割が重複しないと判断した
（`apps/web/src/features/tasks/CalendarBoard.tsx`）。
ガントチャートは要望されなかったため未実装のまま。`due_at`/`due_date`は単一時点の期限であり期間を
表現できないため、実装するなら`docs/schema.sql`の変更（開始日カラムの追加）から検討が必要になる。
