...
### 5. リスト型以外のビュー形式は仕様未定（改修3回目）

ユーザーから「今はリスト型のビューしかないが、それ以外にもビューの形がほしい」との要望があったが、
カンバン/カレンダー/ガントチャートのどれを指しているか、あるいは他の形式かが不明。実装規模が
ビュー種別によって大きく異なる（カンバンは`sort_order`だけで実現できる可能性がある一方、
カレンダー表示は既存のICSカレンダー機能と役割が重複する、ガントは`due_at`/`due_date`だけでは
期間を表現できず`schema.sql`の変更が必要になる可能性がある、等）。
**判断**：仕様を決めずに実装するとやり直しのコストが大きいため実装を見送り、Nestio上に
`manual`タグ付きのサブタスクとして「どの形式が欲しいか」を確認する質問を残した
（`019fcd01-c0cc-76e7-ab29-ec5f230ed071`の子タスク）。
