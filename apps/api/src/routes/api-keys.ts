...
import { issueApiKey, listApiKeys, revokeApiKey, DASHBOARD_SCOPE } from '../api-keys/keys.js';
...
apiKeysRoute.post('/api-keys', async (c) => {
  ...
  // UIの選択肢は「読み取りのみ」「読み書き」「ダッシュボード用」の3択（改修26回目で3つ目を追加）。
  // oauth_tokensに合わせ内部的にはスペース区切りで保存する
  const scope = body.scope === 'write' ? 'read write' : body.scope === DASHBOARD_SCOPE ? DASHBOARD_SCOPE : 'read';
