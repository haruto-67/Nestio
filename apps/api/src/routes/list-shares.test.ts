...
describe('list-shares routes', () => {
  ...
  it('招待→承諾→一覧→解除の一連の流れが動く', async () => {
    ...
    const inviteRes = await app.request('/api/v1/list-shares', {
      method: 'POST',
      headers: { Cookie: `nestio_session=${ownerSession}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ list_id: listId, invited_email: `${editorId}@example.com` }),
    });
    expect(inviteRes.status).toBe(201);
    const share = (await inviteRes.json()) as { id: string; status: string };
    expect(share.status).toBe('pending');

    const incomingRes = await app.request('/api/v1/list-shares/incoming', {
      headers: { Cookie: `nestio_session=${editorSession}` },
    });
    expect(await incomingRes.json()).toHaveLength(1);

    const acceptRes = await app.request(`/api/v1/list-shares/${share.id}/accept`, {
      method: 'POST',
      headers: { Cookie: `nestio_session=${editorSession}` },
    });
    ...
    const outgoingRes = await app.request('/api/v1/list-shares/outgoing', {
      headers: { Cookie: `nestio_session=${ownerSession}` },
    });
    expect(await outgoingRes.json()).toHaveLength(1);

    const revokeRes = await app.request(`/api/v1/list-shares/${share.id}`, {
      method: 'DELETE',
      headers: { Cookie: `nestio_session=${ownerSession}` },
    });
  ...
  it('未登録のメールアドレスへの招待はnot_foundになる', async () => {
    db = createTestDb();
    const ownerId = uuidv7();
    insertTestUser(db, ownerId);
    const listId = insertTestList(db, ownerId);
    const session = insertSession(db, ownerId);
    const app = setupApp(db);

    const res = await app.request('/api/v1/list-shares', {
      method: 'POST',
      headers: { Cookie: `nestio_session=${session}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ list_id: listId, invited_email: 'nobody@example.com' }),
    });
    expect(res.status).toBe(404);
  });

  it('他人のリストは共有できない（forbidden）', async () => {
    db = createTestDb();
    const ownerId = uuidv7();
    ...
    const res = await app.request('/api/v1/list-shares', {
      ...
      body: JSON.stringify({ list_id: listId, invited_email: `${targetId}@example.com` }),
    });
  ...
  it('未認証は401', async () => {
    db = createTestDb();
    const app = setupApp(db);
    const res = await app.request('/api/v1/list-shares/incoming');
